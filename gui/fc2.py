...
a=Label(f1,text=u"サイトURL:", bg ='#a72944').grid(row=0, column=0)
b=Entry(f1,textvariable=urltext,bg ='#a72944',fg='black',width=40).grid(row=0, column=1,padx=10,pady=5)


pagetext=IntVar()
c=Label(f1,text=u"page取得数:", bg ='#a72944').grid(row=1, column=0)
d=Spinbox(f1, from_ = 1, to = 500, increment = 1, bg ='#a72944',textvariable=pagetext, width = 39).grid(row=1, column=1)
#ディレクトリ変更
path_name=""
filename=""
cu=""
...
e=Label(f1,text=u"画像保存先:",bg ='#a72944').grid(row=2, column=0)
f=Button(f1,text="ディレクトリ選択",width=40, bg ='#a72944',activebackground='#c61e42',command=hozon).grid(row=2, column=1,padx=10,pady=5)
f2 = LabelFrame(root, text = "実行履歴", bg ='#a72944', bd=2, relief = GROOVE)
# Listbox の生成
...
def download():
    if urltext.get():
		for i in range(pagetext.get()):
			if i == 0:
				scrape(urltext.get())//rootURI
			else:
				scrape(urltext.get() + u"page-" + str(i) + u".html")//FC2blogURI
    else:
        tkMessageBox.showerror('エラー','URLを入力してください')


Button(root,text="画像取得開始",bg ='#a72944',activebackground='#c61e42',cursor='spider',command=download).pack(side=BOTTOM,pady=3)
